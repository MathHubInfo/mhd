...
class FlushCollectionTest(TestCase):
    ...
    def test_flush_collection(self):
        ...
        self.assertFalse(self.collection_a.is_empty(), 'Check that the first collection is not empty')
        self.assertTrue(self.collection_b.is_empty(), 'Check that the second collection is empty')
        ...
        self.assertJSONEqual(json.dumps(list(GOT_QUERY_NONE)), [], "check that there are no values in collection b")
